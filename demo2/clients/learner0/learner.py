...
class Learner(device.Device):
    ...
    def start_loop(self):
        while True:
            ...
            if os.path.exists("prepare"):
                os.remove("prepare")
                cmd=self.get_twin_content("taskparam")
                fl=fldriver.FL_Driver(cmd.replace("_"," "))
            ...
            if os.path.exists("run") and self._get_twin_conf("status")!="Running":
                ...
                print("run!!!")
                cmd=self.get_twin_content("taskparam")
                fl=fldriver.FL_Driver("python nn.py flakeedge1.com:2333 edge1")
...
def main():
    l1=Learner("learner0","flakeedge1.com",1883,"raspberrypi")
    res=l1.start_edge()
    l1.start_loop()
